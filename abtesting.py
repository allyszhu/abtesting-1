...
def get_stdev(nums):
    ...
    '''
    var = sum(pow(num - get_avg(nums), 2) for num in nums) / (len(nums)-1)
    stdev = pow(var, 0.5)
    return stdev
...
def get_standard_error(a, b):
    ...
    '''
    aye = pow(get_stdev(a), 2) / len(a)
    bee = pow(get_stdev(b), 2) / len(b)
    inside = aye + bee
...
def get_2_sample_df(a, b):
    ...
    '''
    top = pow(get_standard_error(a, b), 4)
    bottom_left_top = pow(round(pow(get_stdev(a), 2)) / len(a), 2)
    bottom_left_bot = (len(a)-1)
    bottom_left = bottom_left_top / bottom_left_bot

    bottom_right_top = pow(round(pow(get_stdev(b), 2)) / len(b), 2)
    bottom_right_bot = (len(b)-1)
    bottom_right = bottom_right_top / bottom_right_bot
    inside = top / (bottom_left + bottom_right)
    df = round(inside)
    return df
...
def row_sum(observed_grid, ele_row):
    sum = 0
    for i in range(0,len(observed_grid[0])):
        sum = sum + observed_grid[ele_row][i]
    return sum
...
def col_sum(observed_grid, ele_col):
    ...
    for i in range(0,len(observed_grid)):
        sum = sum + observed_grid[i][ele_col]
    return sum
...
def get_expected_grid(observed_grid):
    ...
    ** DO NOT modify the parameter, observed_grid. **
    ...
    for i in range(0,len(observed_grid)):
        new = []
        for j in range(0,len(observed_grid[0])):
            new.append(calculate_expected(row_sum(observed_grid, i), 
            col_sum(observed_grid, j), total_sum(observed_grid)))
        grid.append(new)
    return grid
...
def chi2_value(observed_grid):
    ...
    '''
    sum = 0
    eg = get_expected_grid(observed_grid)
    for i in range(0,len(observed_grid)):
        for j in range(0,len(observed_grid[0])):
            chi = pow(observed_grid[i][j] - eg[i][j],2) / eg[i][j]
            sum = sum + chi
